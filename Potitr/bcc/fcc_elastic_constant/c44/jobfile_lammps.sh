...
here=$(pwd)
rm -f output
#  declare -a arr=(-0.007 -0.006 -0.005 -0.004 -0.003 -0.002 -0.001 0.00 0.001 0.002 0.003 0.004 0.005 0.006 0.007 )
declare -a arr=( -0.002 -0.001 0.00 0.001 0.002  )
...
#xxx=$(grep -i 'a_0' /home/masud/job/bcc/output | tail -n1 |  sed -e 's/a_0\(.*\)(.*)/\1/')
#xxx=4.3880
xxx=$(grep -i 'fcclat' ../../output | tail -n1 |  sed -e 's/fcclat\(.*\)(.*)/\1/')
#latticename="Li"
cohesiveenergy=$(grep -i 'E_0' ../../output | tail -n1 |  sed -e 's/E_0\(.*\)(.*)/\1/')
