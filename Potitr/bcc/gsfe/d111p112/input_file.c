...
void input_file(double* lx, double* ly,double* lz,int nx,double a){
double ax=lx[0]/2-0.1;
char ch[60];
char ch1[60];
char ch2[60];
...
//  fprintf(fp,"pair_coeff      * *  /Users/masud/MD/potentials/library_%s.meam %s /Users/masud/MD/potentials/%s.meam  %s\n",ch1,ch,ch,ch);
  ...
  fprintf(fp,"region 1 block %.20lf %.20lf %.20lf %.20lf %.20lf %.20lf\n",0.0, ax,0.0,ly[1],0.0,lz[2]);
  fprintf(fp,"region 2 block %.20lf %.20lf %.20lf %.20lf %.20lf %.20lf\n",ax,lx[0],0.0,ly[1],0.0,lz[2]);
  ...
  fprintf(fp,"%s","fix 1 top setforce NULL 0 0\n");
  ...
  fprintf(fp,"%s", "minimize        1.0e-12 1.0e-12 100000 1000000\n");
