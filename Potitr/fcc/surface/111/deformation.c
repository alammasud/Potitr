...
lx[0]=(1+strain[0])*lx[0];
lx[1]=0.5*strain[5]*lx[1];
lx[2]=0.5*strain[4]*lx[2];


ly[0]=0.5*strain[5]*ly[0];
ly[1]=(1+strain[1])*ly[1];
ly[2]=0.5*strain[3]*ly[2];


  lz[0]=0.5*strain[4]*lz[0];
  lz[1]=0.5*strain[3]*lz[1];
  lz[2]=(1+strain[2])*lz[2];
...
alpha=acos(dotproduct(ly,lz)/(by*bz));
beta=acos(dotproduct(lx,lz)/(bx*bz));
gamma=acos(dotproduct(lx,ly)/(bx*by));
